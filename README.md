...
x Add the HTML Boilerplate to the HTML document.
x Update the document title through the `title` tag content.
x Create the page main heading with the `h1` tag.
x Create an image element with the `img` tag.
